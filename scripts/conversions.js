...
export function rgbToHsl(r, g, b) {
  ...
  s = Math.round(s*100)%101;
  l = Math.round(l*100)%101;

  return [h, s, l];
}

// Coversions with results 
...
export const rgb2Hex = (r,g,b) => {
  ...
  // Combine the components into a single hexadecimal value
  return `#${r}${g}${b}`;
}

export const  rgb2Hsl = (r, g, b) => {
  const clr = rgbToHsl(r,g,b);
  return `hsl(${clr[0]}, ${clr[1]}%,${clr[2]}%)`;
}

export const rgb2Hsb = (r,g,b) => {
    const hsl = rgbToHsl(r,g,b);
    const hsb = hslToHsv(...hsl);
