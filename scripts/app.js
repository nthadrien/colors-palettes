...
function drawColors(colors) {
  ...
  for (let i = 0; i < results.length; i++) {
    if (i < max) {
      ...
      // changing the values of saturation and light;
      current.children[2].value = colors[i][1];
      current.children[3].value = colors[i][1];
      current.children[5].value = colors[i][2];
      current.children[6].value = colors[i][2];
...
function generateProposals() {
  ...
  if (GlobalScheme && GlobalPallete.length > 0) {
    ...
    for (let scheme of list) {
      ...
      container.className = "proposal-row";
